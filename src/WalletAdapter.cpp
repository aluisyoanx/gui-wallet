...
void WalletAdapter::synchronizationProgressUpdated(uint32_t _current, uint32_t _total) {
  m_isSynchronized = false;
  Q_EMIT walletStateChangedSignal(QString("<span style='color: white;'>%1</span><br />Height: %2/%3").arg(tr("SYNCHRONIZING")).arg(_current).arg(_total));
...
void WalletAdapter::updateBlockStatusText() {
  ...
  Q_EMIT walletStateChangedSignal(QString(tr("<span style='color: white;'>%1</span><br />Height: %2<br />%4<br />%3")).
    arg(statusString).
