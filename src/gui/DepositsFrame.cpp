...
void DepositsFrame::depositClicked() 
{
  ...
  /* Deposits launch */
  if (NodeAdapter::instance().getLastKnownBlockHeight() < 36386) 
  {
...
void DepositsFrame::investmentClicked() {
  ...
  if (NodeAdapter::instance().getLastKnownBlockHeight() < 100000) {
    QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Coming Soon"), QtCriticalMsg));
    return;
  }
...
void DepositsFrame::investmentsClicked() {
  ...
  m_ui->m_investmentSelectButton->setStyleSheet("color: white; background-color: #282d31;font-size: 18px;"); 
...
void DepositsFrame::depositsClicked() {
  ...
  m_ui->m_depositSelectButton->setStyleSheet("color: white; background-color: #282d31; font-size: 18px;");
