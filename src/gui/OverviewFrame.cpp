...
OverviewFrame::OverviewFrame(QWidget* _parent) : QFrame(_parent), m_ui(new Ui::OverviewFrame), m_priceProvider(new PriceProvider(this)), m_transactionModel(new RecentTransactionsModel) 
{
  ...
  const QUrl url = QUrl::fromUserInput("http://explorer.aluisyo.network/services/charts/price.png?vsCurrency=usd&days=7&priceDecimals=2&xPoints=12&width=711&height=241");
  ...
  const QUrl url2 = QUrl::fromUserInput("http://explorer.aluisyo.network/services/charts/price.png?vsCurrency=btc&days=7&priceDecimals=6&priceSymbol=btc&xPoints=12&width=711&height=241");
...
void OverviewFrame::walletSynchronized(int _error, const QString& _error_text) 
{
  ...
  m_ui->m_newTransferButton->setStyleSheet("QPushButton#m_newTransferButton {color: #ddd; background-color: #212529; border: 0px solid #343a40;font-family: Lato;font-size: 13px;} QPushButton#m_newTransferButton:hover {color: white; background-color: #212529; border: 0px solid #343a40; font-family: Lato;font-size: 13px;}"); 
  m_ui->m_newDepositButton->setStyleSheet("QPushButton#m_newDepositButton {color: #ddd; background-color: #212529; border: 0px solid #343a40;font-family: Lato;font-size: 13px;} QPushButton#m_newDepositButton:hover {color: white; background-color: #212529; border: 0px solid #343a40; font-family: Lato;font-size: 13px;}"); 
  m_ui->m_newMessageButton->setStyleSheet("QPushButton#m_newMessageButton {color: #ddd; background-color: #212529; border: 0px solid #343a40;font-family: Lato;font-size: 13px;} QPushButton#m_newMessageButton:hover {color: white; background-color: #212529; border: 0px solid #343a40; font-family: Lato;font-size: 13px;}"); 
  showCurrentWallet();
  walletSynced = true;
...
void OverviewFrame::actualBalanceUpdated(quint64 _balance) 
{
  ...
  quint64 pendingInvestmentBalance = WalletAdapter::instance().getPendingInvestmentBalance();  
  quint64 totalBalance = pendingDepositBalance + actualDepositBalance + actualBalance + pendingBalance + pendingInvestmentBalance + actualInvestmentBalance;
  m_ui->m_totalBalanceLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + pendingBalance) + " ANX");
  m_priceProvider->getPrice();   
}
...
void OverviewFrame::pendingBalanceUpdated(quint64 _balance) 
{
  ...
  quint64 pendingInvestmentBalance = WalletAdapter::instance().getPendingInvestmentBalance();  
  quint64 totalBalance = pendingDepositBalance + actualDepositBalance + actualBalance + pendingBalance + pendingInvestmentBalance + actualInvestmentBalance;
  m_ui->m_totalBalanceLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + actualBalance) + " ANX");
  m_priceProvider->getPrice(); 
}
...
void OverviewFrame::actualDepositBalanceUpdated(quint64 _balance) 
{
  ...
  quint64 pendingInvestmentBalance = WalletAdapter::instance().getPendingInvestmentBalance();  
  quint64 totalBalance = pendingDepositBalance + actualDepositBalance + actualBalance + pendingBalance + pendingInvestmentBalance + actualInvestmentBalance;
  m_ui->m_totalDepositLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + pendingDepositBalance) + " ANX");
  m_priceProvider->getPrice(); 
}
...
void OverviewFrame::actualInvestmentBalanceUpdated(quint64 _balance) 
{
  ...
  m_ui->m_totalInvestmentLabel->setText(CurrencyAdapter::instance().formatAmount(pendingInvestmentBalance + actualInvestmentBalance) + " ANX");
  m_priceProvider->getPrice(); 
}
...
void OverviewFrame::pendingDepositBalanceUpdated(quint64 _balance) 
{
  ...
  quint64 pendingInvestmentBalance = WalletAdapter::instance().getPendingInvestmentBalance();  
  quint64 totalBalance = pendingDepositBalance + actualDepositBalance + actualBalance + pendingBalance + pendingInvestmentBalance + actualInvestmentBalance;
  m_ui->m_totalDepositLabel->setText(CurrencyAdapter::instance().formatAmount(_balance + actualDepositBalance) + " ANX");
  m_priceProvider->getPrice(); 
}
...
void OverviewFrame::aboutClicked() 
{
  if (subMenu != 4) {
    ...
    m_ui->m_subButton5->setEnabled(false);    
    m_ui->m_subButton1->setText("About Aluisyo");
